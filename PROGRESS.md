...
| 用途 | URL |
|------|-----|
| メインアプリ | https://invoice-ocr-tawny.vercel.app/ |
| **営業用LP（要トークン）** | https://invoice-ocr-tawny.vercel.app/sales?t=deffa15c3155d651244fa73ae8a04556 |
...
### 営業ページ群（2026-03-30 追加）
- [x] `/sales` — 営業LP（PDFビューア付き・全ページへのナビ）
- [x] `/pricing` — ライト¥5,000 / ヘビー¥10,000 + 比較表
- [x] `/security` — データフロー図・3つの約束・他手段との比較表
- [x] `/guide` — 6ステップ操作説明・ビフォーアフター試算
- [x] `/faq` — 15問アコーディオン（4カテゴリ）
- [x] `/denied` — トークンなしアクセス時の拒否ページ

### セキュリティ
- [x] `proxy.ts` にトークン保護を統合（`/sales`・`/security`・`/guide`・`/faq`・`/pricing`）
